...
export default class DataEsrConnecteur extends BaseConnecteur {
  ...
  async rechercher(query, options = {}) {
    ...
    const url =
      `${ENDPOINT_STRUCTURES}?q=${encodeURIComponent(terme)}` +
      `&limit=${limite}`;
    ...
    const resultats = (donnees.results ?? []).map((item) =>
      this._mappageStructure(item),
    );
  ...
  async detailler(id) {
    ...
    const url =
      `${ENDPOINT_STRUCTURES}?q=${encodeURIComponent(idPropre)}&limit=1`;
