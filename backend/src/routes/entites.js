...
export default async function entitesRoutes(fastify) {
   ...
   */
  fastify.get('/:type/:id/foncier', { preHandler: [optionalAuth] }, async (request, reply) => {
    const { type, id } = request.params

    if (!TYPES_ENTITE_ACCEPTES.has(type)) {
      return reply
        .code(400)
        .send({ error: "Type d'entité invalide. Valeurs acceptées : personne, organisation" })
    }
   ...
   */
  fastify.get('/:type/:id/cadastre', { preHandler: [optionalAuth] }, async (request, reply) => {
    const { type, id } = request.params
   ...
   */
  fastify.get('/:type/:id/urbanisme', { preHandler: [optionalAuth] }, async (request, reply) => {
    const { type, id } = request.params
