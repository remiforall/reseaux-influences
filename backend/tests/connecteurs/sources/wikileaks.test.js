/**
 * Tests du stub WikiLeaks (désactivé).
...
const { default: WikiLeaksConnecteur } = await import(
  '../../../src/connecteurs/sources/wikileaks.js'
);

describe('WikiLeaksConnecteur — STUB DÉSACTIVÉ', () => {
  it('rechercher() retourne un tableau resultats vide', async () => {
    const connecteur = new WikiLeaksConnecteur();
  ...
  it('ne declenche aucun appel reseau (fetch non appele)', async () => {
    // Si fetch était appelé, il échouerait en environnement de test sans mock
    // Le stub ne doit pas avoir besoin de fetch
