...
const { default: IcijOffshoreLeaksConnecteur } = await import(
  '../../../src/connecteurs/sources/icij-offshore-leaks.js'
);
...
function creerConnecteurAvecIndex(nodes) {
  ...
  for (const node of nodes) {
    ...
    const nom = node.name ?? '';
    const cle = nom.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
