...
function creerLienMock(id, entiteA, entiteB, typeLienCode = 'MANDAT_ELECTIF', statut = 'VALIDE') {
  return {
    ...
    personneA: entiteA.type === 'personne'
      ? { id: entiteA.id, nom: entiteA.nom, prenom: entiteA.prenom ?? null, rolePrincipal: null, pays: null, photoUrl: null }
      : null,
    organisationA: entiteA.type === 'organisation'
      ? { id: entiteA.id, nom: entiteA.nom, sigle: null, typeOrganisation: 'AUTRE', pays: null, logoUrl: null }
      : null,
    siteWebA: entiteA.type === 'site_web'
      ? { id: entiteA.id, domaine: entiteA.domaine, hebergeurProbable: null }
      : null,
    personneB: entiteB.type === 'personne'
      ? { id: entiteB.id, nom: entiteB.nom, prenom: entiteB.prenom ?? null, rolePrincipal: null, pays: null, photoUrl: null }
      : null,
    organisationB: entiteB.type === 'organisation'
      ? { id: entiteB.id, nom: entiteB.nom, sigle: null, typeOrganisation: 'AUTRE', pays: null, logoUrl: null }
      : null,
    siteWebB: entiteB.type === 'site_web'
...
describe("GET /api/graphe/ego/:entiteId — ego-network 2 sauts (Personne)", () => {
  beforeAll(() => {
    ...
    mockLienFindMany.mockImplementation(({ where }) => {
      const or = where?.OR ?? [];
      if (or.some((c) => c.personneAId === 'personne-racine-id' || c.personneBId === 'personne-racine-id')) {
